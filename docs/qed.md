...
QED is the theory of two species of quantum, the electron and the photon. It therefore starts from the two free Lagrangians the [Action and Lagrangians](qft-action.md) page read off the field table, one per species.
...
and the kinetic term splits into the free kinetic term plus $-q\bar\psi\gamma^\mu\psi\,A_\mu$. This is the minimal coupling that [The Dirac Equation §1](dirac-equation.md#_1-spin) used to introduce charge in the first place. The next two sections justify why a gauge principle demands exactly this substitution. The term above is what that principle forces.
...
The family of re-phasings has a standard name. As $\alpha$ runs over all angles, the factors $e^{i\alpha}$ trace out a circle, and a circle of transformations is the group called **U(1)**. The group is unitary because $|e^{i\alpha}| = 1$, and the $1$ is there because a single angle labels every member. This section's global symmetry is a U(1) symmetry.

That symmetry also motivates the next step. If the absolute phase is unobservable, singling out one common phase at every point is arbitrary. Nothing in the physics prefers it. Promote $\alpha$ to a function $\alpha(x)$, letting each point choose its own phase, and the derivative no longer commutes past the phase. The derivative $\partial_\mu\big(e^{i\alpha(x)}\psi\big)$ leaves a $\partial_\mu\alpha(x)$ term behind, and the Lagrangian is no longer invariant. Restoring invariance forces a new field into the theory, the gauge field $A_\mu$, and with it a rule that trades the ordinary derivative for the covariant one. That demand is the local gauge symmetry of the next section, and it is exactly where the photon and the interaction term $-q\bar\psi\gamma^\mu\psi\,A_\mu$ come from.

## Local Gauge Symmetry U(1)

The last section's symmetry used one phase for all of spacetime, and that choice is arbitrary. If the absolute phase is unobservable at every point, then nothing in the physics prefers one point's phase over another's. The natural next move is to let each point choose its own. Promote the constant angle to a function $\alpha(x)$, and ask that the theory remain unchanged under
...
so replacing $\partial_\mu$ by $D_\mu$ restores the symmetry. The term $\bar\psi\gamma^\mu D_\mu\psi$ is invariant term by term, exactly as the global version was. The compensating field $A_\mu$ is not a third field. It is the photon field of the recap. Expanding $D_\mu$,

$$\bar\psi\,i\hbar\gamma^\mu D_\mu\psi \;=\; \bar\psi\,i\hbar\gamma^\mu\partial_\mu\psi \;-\; q\,\bar\psi\gamma^\mu\psi\,A_\mu ,$$

and the extra term is precisely the interaction term of the recap, the photon field $A_\mu$ coupled to the electron current $\bar\psi\gamma^\mu\psi$, with the charge $q$ setting the strength. Local gauge invariance therefore explains why the photon couples to charge. The field the symmetry requires is the photon field $A_\mu$ already in the theory, not a new one. The photon still needs a kinetic term of its own, the Maxwell term. Its gauge invariance is the subject of the next section.
...
The potential may oscillate, but both $\mathbf E$ and $\mathbf B$ vanish. This is what *pure gauge* means here. The potential is nonzero, yet it describes zero physical field.[^longitudinal] Consequently, a plane wave with a nonzero electromagnetic field must have
...
The gauge freedom removes one more. A gauge transformation shifts $A_\mu$ by a gradient,[^gradient] and the gradient of the same plane wave is proportional to $k_\mu$. The transformation therefore acts on the polarization as
...
and for the time component this is identically zero, $\pi^0 = -F^{00} = 0$. The entry $F^{00}$ is just $\partial^0 A^0$ subtracted from itself. This is how the antisymmetry of $F$ empties its diagonal, and it makes the zero structural. Imposing the canonical commutator on $A_0$ would read $[\hat A_0,\, 0] = i\hbar\,\delta^3$, a contradiction. The construction cannot take its first step. The three spatial components do pair with conjugate momenta, and in fact $\pi^i = E^i$, the electric field itself. The fourth component has none.

The proof is complete. The rest of this subsection explains the meaning of the zero, and that meaning determines the form of the repair. Physically, $\pi^0 = 0$ states that $A_0$ is not independent data. The $\nu = 0$ component of the free equation is Gauss's law, $\nabla\cdot\mathbf E = 0$. It contains no time derivative, so it does not describe time evolution. It constrains $A_0$ to the other components at each instant. The missing conjugate momentum is the Hamiltonian form of the same fact. This is half of the overcounting stated at the top of the section, the fact that four components hold only three dynamical ones. The other half is the gauge redundancy of the first subsection, which concerns the descriptions rather than the dynamics. The distinction matters because the repair uses the two differently. The redundancy justifies modifying the Lagrangian. The missing velocity is what the modification must supply.

### Fixing the gauge

The repair acts on the Lagrangian before quantization. Its goal comes from the previous subsection. The construction stalled because $\dot A_0$ never appears in the Maxwell Lagrangian, so $A_0$ has no conjugate momentum and the canonical pairing $[\hat A_\mu, \hat\pi^\nu] = i\hbar\,\delta^\nu{}_\mu\,\delta^3$ cannot be imposed. The repair must give every component of $A_\mu$ a velocity, the time component included. Modifying the Lagrangian needs justification, since the equations of motion, the Hamiltonian, and the scattering rules are all derived from it. The justification is the redundancy of the first subsection. Only gauge-invariant quantities are physical, and these are built entirely from $F_{\mu\nu}$. The four components of $A_\mu$ include combinations that no such quantity depends on. A term that reorganizes the redundant descriptions can therefore be added without changing any observable.

The reorganization is built around the Lorenz condition[^lorenz] $\partial_\mu A^\mu = 0$. A gauge condition must meet two requirements, and this one meets both. It is covariant, since the left-hand side is a full contraction and treats time and space alike, so the condition takes the same form in every Lorentz frame. It can also always be imposed. Under $A_\mu \to A_\mu + \partial_\mu\chi$ the divergence changes as $\partial\cdot A \to \partial\cdot A + \Box\chi$, so choosing $\chi$ to solve $\Box\chi = -\,\partial\cdot A$ produces a potential that obeys the condition. Some freedom survives, because any further $\chi$ with $\Box\chi = 0$ keeps the condition. For plane waves, this residue is precisely the shift $\varepsilon \to \varepsilon + \alpha k$ of the first subsection. This subsection does not remove it. The next subsection removes the residue at the level of states rather than fields, by the physical-state condition.

For classical solutions, reachability settles the question, since one can simply restrict attention to fields that obey the condition. On the operators of the quantum theory, imposing it directly fails again. The identity $\partial\cdot\hat A = 0$ contradicts the canonical algebra exactly as $\pi^0 = 0$ did.[^operator-lorenz] The condition therefore enters through the Lagrangian, as a **gauge-fixing term**

$$\mathcal{L}_\text{gf} = -\tfrac{1}{2}\,(\partial_\mu A^\mu)^2 .$$

The term has the two required properties. It changes no physics, because it vanishes on every Lorenz-obeying field. Within each gauge orbit it only changes which representative the theory uses. It also supplies the missing velocity. Among its pieces is $-\tfrac12(\partial_0 A_0)^2$, the term containing $\dot A_0$ that the Maxwell Lagrangian lacks. With it, the conjugate momentum of the time component becomes $\pi^0 = -\dot A^0$ instead of zero. All four components now have a conjugate momentum, and the construction of the previous subsection can proceed. The term also decouples the components. Expand the Maxwell term,
...
so the three spacelike families carry the standard $[\hat a, \hat a^\dagger] = 1$ algebra and the timelike family carries it with a minus sign. The sign is inherited from the kinetic term. For the time component the combination $-\tfrac12(\partial_\mu A_\nu)(\partial^\mu A^\nu)$ gives $-\tfrac12(\partial A_0)^2$, the negative of a scalar's kinetic term, because $g^{00} = +1$ while $g^{ii} = -1$. That sign flows through the construction into the algebra. Substituting the expansion into the Hamiltonian, by the same orthogonality steps as the scalar case, gives per momentum
...
and the polarization part of the sum is the completeness relation of the basis. The four vectors $\varepsilon^{(\lambda)}$ span the component space. Summing their outer products with the weights $\eta_\lambda$ reconstructs the metric,
...
in momentum space, and this is the photon propagator. No mass term appears in the denominator because the photon is massless. The only pole sits at $k^2 = 0$. The tensor $g_{\mu\nu}$ carries the vector structure, one index for each end of the line. Its sign pattern records the same timelike bookkeeping. An internal photon line effectively sums over all four polarization modes, with the unphysical pair canceling in every gauge-invariant result. This is why diagram calculations may use $-g_{\mu\nu}$ as the polarization sum on internal lines. This is the rule the [Feynman Rules page](feynman-rules.md) needs. With it the free-field input for the QED rules is complete: the electron propagator from the spinor construction, the photon propagator from this one, and the interaction term supplying the vertex.

That is QED as a Lagrangian theory, with both of its fields quantized. The [next page](lagrangian-to-experiment.md) says where such a Lagrangian leads an experiment. The pages after, [Perturbation Theory](perturbation-theory.md) and [Feynman Rules for QED](feynman-rules.md), read the scattering amplitudes out of this Lagrangian.

[^gupta]: The covariant statement is the **Gupta–Bleuler condition**. The Lorenz condition is imposed on physical states, using only the annihilation half of the operator, as $\big(\partial_\mu\hat A^\mu\big)^{(+)}\,\lvert\text{phys}\rangle = 0$. For $k$ along $z$ it pairs the timelike and longitudinal modes as $\big(\hat c_0 - \hat c_3\big)\lvert\text{phys}\rangle = 0$, so the two operators act identically on every physical state. Their Hamiltonian terms $\hat c_3^\dagger\hat c_3 - \hat c_0^\dagger\hat c_0$ have equal and opposite expectation values there. The construction is standard quantum field theory. This site uses only its conclusion, that the unphysical pair cancels in every observable, because the diagram rules build that cancellation into the $-g_{\mu\nu}$ polarization sum of internal lines.

[^gradient]: The gradient form follows from what a gauge transformation is, namely a shift of the potential that changes nothing physical. The physical content lives in the field strength $F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu$, the container of $\mathbf E$ and $\mathbf B$. A general four-vector shift $\chi_\mu$ changes $F_{\mu\nu}$ by $\partial_\mu\chi_\nu - \partial_\nu\chi_\mu$. That is a different electromagnetic field, not a re-description of one. A gradient shift leaves $F_{\mu\nu}$ unchanged because $F_{\mu\nu}$ is antisymmetric in $\mu \leftrightarrow \nu$, and an antisymmetric combination vanishes on a symmetric input. For $\chi_\mu = \partial_\mu\chi$ the extra terms are $\partial_\mu\partial_\nu\chi - \partial_\nu\partial_\mu\chi$, which vanish since mixed partial derivatives commute. Locally the converse holds as well, so the gradient shifts are the complete redundancy, not merely one example of it. This is the same transformation law as in the Local Gauge Symmetry section, with the parameter renamed and rescaled ($\chi$ here, $-\hbar\alpha/q$ there). There the failure to be canceled was itself a gradient, $\partial_\mu\alpha$, so only a gradient shift could cancel it. The shift that compensates the derivative is exactly the shift that leaves the field strength unchanged.

[^operator-lorenz]: The incompatibility in one line. In the gauge-fixed theory the canonical momentum is $\hat\pi^\mu = -\partial^0\hat A^\mu$, so the divergence splits as $\partial_\mu\hat A^\mu = \partial_0\hat A^0 + \partial_i\hat A^i = -\hat\pi^0 + \partial_i\hat A^i$. Were $\partial\cdot\hat A = 0$ an operator identity, its commutator with $\hat A_0$ would have to vanish. The equal-time algebra $[\hat A_\mu(t,\mathbf x), \hat\pi^\nu(t,\mathbf y)] = i\hbar\,\delta^\nu{}_\mu\,\delta^3(\mathbf x-\mathbf y)$ instead gives $[\partial\cdot\hat A(t,\mathbf x), \hat A_0(t,\mathbf y)] = -[\hat\pi^0(t,\mathbf x), \hat A_0(t,\mathbf y)] = i\hbar\,\delta^3(\mathbf x-\mathbf y)$. The $\partial_i\hat A^i$ piece contributes nothing, because field operators commute with each other at equal times. The identity and the algebra cannot both hold. The condition therefore cannot be imposed as an operator identity. It is imposed in the Lagrangian here, and on the states of the theory in the next subsection.
...
[^transverse]: The condition $k\cdot\varepsilon = 0$ is the covariant statement of transversality, written for the 4-potential $A^\mu = (\phi, \mathbf A)$. It involves the scalar potential's component $\varepsilon^0$. The familiar 3D form, $\mathbf k\cdot\mathbf E = 0$ giving $E_z = 0$, is a statement about the fields $\mathbf E$ and $\mathbf B$, which have no time component. That is why the two versions look different. For the lightlike $k = (1,0,0,1)$ the dot product $k\cdot\varepsilon = k^0\varepsilon^0 - k^3\varepsilon^3$ reduces to $\varepsilon^0 = \varepsilon^3$. The direction that satisfies it, the diagonal $(1,0,0,1)$, is exactly the pure-gauge mode with $F_{\mu\nu} = 0$. The two statements agree. The physical polarizations $(0,1,0,0)$ and $(0,0,1,0)$ have zero $z$-component in the potential and in the field alike.
