...
[^rulebook]: An internal photon contributes $-ig_{\mu\nu}/(r^2+i\epsilon)$ in Feynman gauge, derived in [Perturbation Theory](perturbation-theory.md#the-qed-photon-propagator) from the gauge-fixed field introduced on the [QED page](qed.md#quantizing-the-gauge-fixed-field). Incoming and outgoing positrons contribute $\bar v(p)$ and $v(p')$, respectively, using the negative-frequency solutions in the Dirac equation's [general solution](dirac-equation.md). These conventions agree with [David Tong's QED notes](https://www.damtp.cam.ac.uk/user/tong/qft/qfthtml/S6.html).
