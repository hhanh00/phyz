...
The upgrade to a field is one change of integration variable. A field $\phi(x)$ is a continuum of coordinates — one value $\phi(t, \mathbf x)$ at every point of space — so what gets integrated is not a function of time but a **Lagrangian density** $\mathcal{L}(\phi, \partial_\mu\phi)$ over all of spacetime,
