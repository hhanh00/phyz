...
The same construction, stated once without the spinor dressing because the construction below builds it that way, takes the scalar field $\hat\phi$ (normalization suppressed, $k\cdot x = E_k t - \mathbf k\cdot\mathbf x$, $E_k = \sqrt{\mathbf k^2 + m^2}$):
...
so $\hat a$ annihilates a particle and $\hat b^\dagger$ creates an antiparticle inside the field, while $\hat a^\dagger$ creates a particle and $\hat b$ annihilates an antiparticle inside its adjoint. The doubled coefficients make charge visible. A field that equals its own adjoint, a real field with $\hat\phi^\dagger = \hat\phi$, has no $\hat b$'s at all and describes a particle that is its own antiparticle; the field needs both pairs only when particle and antiparticle are distinct, opposite charges. One honest note: charge here is a *quantum number*, the conserved label that separates particle from antiparticle, and nothing so far shows it to be the electric charge of electromagnetism. That identification waits until the field couples to $A_\mu$.
